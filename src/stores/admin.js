    ...
    async getAppointmentDailyHistory(date) {
      try {
        ...
        const axiosRequest = await axios.get(
          `${import.meta.env.VITE_VUE_APP_API_URL}/user/admin/appointments-daily-history?date-selected=${date}`,
    ...
    async modifyAppointment(values) {
      try {
        const axiosRequest = await axios.patch(
          `${import.meta.env.VITE_VUE_APP_API_URL}/user/admin/modify-existing-appointment`,
          { values },
          { withCredentials: true }
        );
